...
export class AuthRepository {
    ...
    async comparePassword(email: string, password: string): Promise<IUser> {
        try {
            const result = await this.userRespository.compareUserByPassword(email, password)
            return result
        } catch (error) {
            throw error
        }
    ...
    async getToken(userId: string, deviceId: string, token: string) {
        try {
            const result = await this.tokenRespository.getToken(userId, deviceId, token);
            return result
        } catch (error) {
            throw {
                status: 500,
    ...
    async revoke(userId: string, deviceId: string) {
        try {
            const result = await this.tokenRespository.revokeTokenBasedOnDeviceId(userId, deviceId);
            return result
        } catch (error) {
            throw error
        }
