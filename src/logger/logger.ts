...
class Logger {
    ...
    private writeLog(level: LogLevel, message: string, additionalInfo?: any): void {
        ...
        const fullLogMessage = additionalInfo ? `${logMessage} - ${JSON.stringify(additionalInfo)}` : logMessage;
    ...
    private getColoredMessage(level: LogLevel, timestamp: string, message: string, additionalInfo?: any): string {
        const logLevels = {
        ...
        const additionalLabel = additionalInfo ? chalk.gray(` - ${JSON.stringify(additionalInfo)}`) : '';
