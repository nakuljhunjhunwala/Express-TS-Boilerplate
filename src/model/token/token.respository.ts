...
export class TokenModelRespository {
    ...
    async getToken(userId: string, deviceId: string, refreshToken: string) {
        return await Token.findOne({ userId: userId, deviceId: deviceId, token: refreshToken, valid: true })
    }
    ...
    async revokeTokenBasedOnDeviceId(userId: string, deviceId: string) {
        return await Token.updateMany({ userId: userId, deviceId: deviceId, valid: true }, { valid: false })
    }
